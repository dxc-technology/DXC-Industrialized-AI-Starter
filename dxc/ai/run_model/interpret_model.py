...
def Explanation_Dashboard(global_explanation, model, x_train, x_test, y_train, y_test, explantion_data = None):
    if explantion_data == 'Training':
        ExplanationDashboard(global_explanation, model, datasetX=x_train, datasetY=y_train)
    else:
        ExplanationDashboard(global_explanation, model, datasetX=x_test, datasetY=y_test)
