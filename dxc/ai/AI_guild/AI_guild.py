...
# To apply the badges from new badger site
def guild_member_should_apply_for_badge2(guild_member_roles, ai_badge):

  guild_role_badges = {
    AI_Guild_Role.PROJECT_MANAGER : ['Create a Data Story', 'Run Agile Transformation'],
    AI_Guild_Role.DATA_SCIENTIST : ['Run AI Experiment', 'Perform AI Forensics'],
    AI_Guild_Role.DATA_ENGINEER : ['Build AI Data Pipelines', 'Build AI Utility Services'],
    AI_Guild_Role.ALL: ['Create a Data Story', 'Build AI Data Pipelines', 'Run AI Experiment', 'Perform AI Forensics', 'Build AI Utility Services', 'Run Agile Transformation']
    }
  ...
  for role in guild_member_roles:
      #apply for the badge if a matching role is found
      if ai_badge in guild_role_badges[role]: 
        apply_for_badge = True
  return apply_for_badge

def apply_for_an_ai_badge(ai_guild_profile, ai_badge):

  if dt.strptime(ai_guild_profile["bootcamp participation date"], '%m/%d/%Y') >= dt.strptime('03/15/2022', '%m/%d/%Y'):
    api_end_point = r'https://dxc-badging-backend-api.azurewebsites.net/createnewaistarterassetions'
    for i, member_details in ai_guild_profile['guild_members'].items():
      if guild_member_should_apply_for_badge2(member_details['roles'], ai_badge):
        data = {"userEmail":member_details['badge_applicant_email'],
                "badgeName": ai_badge,
                "badgeStatus": '5f776f556289f17659874f2e',
                "evidenceLink": ai_guild_profile['badge_evidence'],
                "adminID": '614a4946e59455f33d9c3b1a',
                "comments": 'Badge applied from from AI Starter Notebook'
                }       
        response = requests.post(api_end_point, json = data)
        print('Badge Status for user email %s : %s.' %(member_details['badge_applicant_email'], response.text))
        print(' ')
